    ...
    if (isEditorOpen) {
        if (isWorldPaused) {
            ...
            if (Input::GetKey(KC_LALT)) {
                ...
                auto rotEuler = glm::eulerAngles(cameraRot);
                rotEuler.y += dragDelta.x * Time::GetUnscaledDeltaTime();
                cameraRot = glm::quat(
                    glm::rotate(cameraRot, dragDelta.x * Time::GetUnscaledDeltaTime(), glm::vec3(0.f, -1.f, 0.f)));
            }
