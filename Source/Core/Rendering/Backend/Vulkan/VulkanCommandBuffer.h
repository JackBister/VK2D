...
public:
	...
	VulkanCommandBuffer(VkCommandBuffer buffer, std::allocator<uint8_t> * allocator) : CommandBuffer(allocator), buffer(buffer) {}
	...
	virtual void CmdSetScissor(uint32_t firstScissor, uint32_t scissorCount, CommandBuffer::Rect2D const * pScissors) override;
	virtual void CmdSetViewport(uint32_t firstViewport, uint32_t viewportCount, CommandBuffer::Viewport const * pViewports) override;
