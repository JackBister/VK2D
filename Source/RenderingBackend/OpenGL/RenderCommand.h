...
#include "../Abstract/ResourceCreationContext.h"

struct RenderCommand {
    enum Type { NOP, ABORT, CREATE_RESOURCE, EXECUTE_COMMAND_CONTEXT, SWAP_WINDOW };
    ...
    struct ExecuteCommandContextParams {
        ...
        CommandBuffer * ctx;
        ExecuteCommandContextParams(CommandBuffer * ctx, std::vector<SemaphoreHandle *> waitSem,
    ...
    using RenderCommandParams =
        std::variant<NopParams, AbortParams, CreateResourceParams, ExecuteCommandContextParams, SwapWindowParams>;
    ...
    RenderCommand(RenderCommand const & rc) { assert(false); }
    RenderCommand(RenderCommand && rc) : params(std::move(rc.params)) {}
    RenderCommand(RenderCommandParams && params) : params(std::move(params)) {}

    void operator=(RenderCommand const & rc) { assert(false); }
    void operator=(RenderCommand && rc) { params = std::move(rc.params); }
};
